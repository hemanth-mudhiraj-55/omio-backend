...
function normalizeChannelKey(channel) {
  return String(channel || 'general').trim().toUpperCase().replace(/[^A-Z0-9]+/g, '_');
}

function getChannelConfig(channel) {
  const channelKey = normalizeChannelKey(channel);
  const fallbackInbox = process.env.CONTACT_INBOX || 'hello@omio.world';
  const fallbackFrom = process.env.SMTP_FROM;

  return {
    inbox: process.env[`CONTACT_INBOX_${channelKey}`] || fallbackInbox,
    from: process.env[`SMTP_FROM_${channelKey}`] || fallbackFrom,
  };
}
...
async function sendContactEmails({ name, email, channel, subject, message, id }) {
  ...
  const { inbox, from } = getChannelConfig(channel);
...
module.exports = { sendContactEmails, getChannelConfig };
